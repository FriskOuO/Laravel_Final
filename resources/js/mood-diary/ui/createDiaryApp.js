...
function createCard(diary, { guest = false } = {}) {
    ...
    return `
        <article class="group card-base flex flex-col justify-between">
            <div class="space-y-5">
            ...
            <div class="mt-8 flex items-center justify-between gap-3 pt-6 border-t border-slate-100 dark:border-slate-800/50">
        ...
        <div class="relative py-12 overflow-hidden">
            ...
            <section class="text-center space-y-10 relative z-10 fade-in py-16">
                ...
                <div class="flex flex-wrap justify-center gap-6 pt-6">
                    ...
                    <button type="button" class="rounded-full border-2 border-slate-200 dark:border-slate-800 px-10 py-5 text-lg font-black transition hover:bg-slate-50 dark:hover:bg-slate-800 text-main" data-jump-preview>
                        ${t('browse_public')}
                    </button>
                </div>
            </section>
...
function adminView(state) {
    return `
        <section class="space-y-12 py-12 fade-in">
            ...
            <div class="card-base">
                <h3 class="text-2xl font-black text-main mb-6">管理功能的詳細內容</h3>
                <p class="text-lg text-sub leading-relaxed">管理控制台目前處於預覽階段。未來將支援使用者權限管理、內容審核以及全站數據匯出功能。</p>
            </div>
        </section>
    `;
}
...
export function createDiaryApp(root) {
    ...
    function render() {
        ...
        let content = '';
        if (state.view === 'auth' && !isAuth) content = authView(state);
        else if (state.view === 'form' && isAuth) content = formView(state);
        else if (state.view === 'detail') content = detailView(state, !isAuth);
        else if (state.view === 'admin' && isAuth) content = adminView(state);
        else if (isAuth) content = listView(state);
        else content = homeView(state);
                    ...
                    <div class="nav-inner">
                        <div class="flex items-center gap-10">
                            ...
                            <nav class="hidden md:flex gap-8">
                                ...
                                ${isAuth ? `<button class="text-sm font-black tracking-widest uppercase transition ${state.view === 'admin' ? 'text-main border-b-2 border-main' : 'text-muted hover:text-main'}" data-nav="admin">${t('admin')}</button>` : ''}
                            </nav>
                        </div>
                        <div class="flex items-center gap-6">
                            ...
                            ${isAuth ? `
                                <div class="flex items-center gap-4">
                                    <span class="hidden sm:inline text-xs font-black text-muted uppercase tracking-widest">${state.auth.currentUser?.name}</span>
